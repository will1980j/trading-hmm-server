...
def get_all_alerts(db):
    ...
    try:
        ...
        if db and hasattr(db, 'conn') and db.conn:
            ...
            try:
                ...
                if recent_trades < 10:
                    performance.append({
                        'type': 'low_activity',
                        'message': f'Only {recent_trades} trades in the last 7 days',
                        'priority': 'medium'
                    })
                ...
                if avg_mfe < 0:
                    performance.append({
                        ...
                        'message': f'Raw data shows {avg_mfe:.2f}R average - optimization needed',
                        'priority': 'medium'
                    })
                ...
                performance.append({
                    'type': 'database_error',
                    'message': 'Database query failed - check connection',
                    'priority': 'low'
                })
        ...
        return {
            'performance': [{'type': 'system_error', 'message': 'Alert system offline', 'priority': 'low'}],
            'system': [],
            'opportunities': []
        }
