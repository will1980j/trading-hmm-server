 ...
 * All metrics from real APIs or marked as locked
...
class MainDashboard {
    ...
    /**
     * Fetch dashboard data from /api/automated-signals/dashboard-data
     * Populates: lifecycle signals, aggregated stats, session metrics, signal quality
     */
    ...
    /**
     * Fetch stats from /api/automated-signals/stats-live
     * Populates: today's signals count, last signal time, webhook health
     */
    async fetchStats() {
        try {
    ...
    calculateStats() {
        ...
        if (completed.length > 0) {
            ...
            this.data.stats.losses = losses;
            this.data.stats.win_rate = completed.length > 0 ? wins / completed.length : 0;
    ...
    /**
     * Filter signals by status (lifecycle logic)
     * Returns: { active: [], closed: [], cancelled: [], invalid: [] }
     */
    ...
    renderPrimaryKPIs() {
        ...
        if (rDistEl) {
            rDistEl.textContent = rDist.toFixed(2) + 'σ';
        }
    }
    ...
    createSignalCard(signal) {
        ...
        const directionClass = direction === 'Bullish' ? 'bullish' : direction === 'Bearish' ? 'bearish' : '';
        ...
        return `
            <div class="signal-card ${directionClass}">
                <div class="signal-header">
                    <span class="signal-direction ${directionClass}">${direction}</span>
                    <span class="signal-time">${signal.time || signal.timestamp || 'N/A'} | ${session}</span>
                </div>
                <div class="signal-details">
                    <div class="signal-detail">
                        <span class="signal-detail-label">Entry:</span>
                        <span class="signal-detail-value">${entryPrice}</span>
                    </div>
                    <div class="signal-detail">
                        <span class="signal-detail-label">SL:</span>
                        <span class="signal-detail-value">${slPrice}</span>
                    </div>
                    <div class="signal-detail">
                        <span class="signal-detail-label">BE:</span>
                        <span class="signal-detail-value">${beAchieved}</span>
                    </div>
                    <div class="signal-detail">
                        <span class="signal-detail-label">MFE (No BE):</span>
                        <span class="signal-detail-value">${mfeNoBE}</span>
                    </div>
                    <div class="signal-detail">
                        <span class="signal-detail-label">MFE (BE):</span>
                        <span class="signal-detail-value">${mfeBE}</span>
                    </div>
                    <div class="signal-detail">
                        <span class="signal-detail-label">Duration:</span>
                        <span class="signal-detail-value">${duration}</span>
                    </div>
                    <div class="signal-detail">
                        <span class="signal-detail-label">R-Multiple:</span>
                        <span class="signal-detail-value">${rMultiple.toFixed(2)}R</span>
                    </div>
                    <div class="signal-detail">
                        <span class="signal-detail-label">Risk:</span>
                        <span class="signal-detail-value">${riskDistance}</span>
                    </div>
                </div>
            </div>
        `;
    ...
    createTradeCard(trade) {
        ...
        return `
            <div class="trade-card ${directionClass}">
                <div class="signal-header">
                    <span class="signal-direction ${directionClass}">${direction}</span>
                    <span class="signal-time">${trade.time || trade.timestamp || 'N/A'}</span>
                </div>
                <div class="signal-details">
                    <div class="signal-detail">
                        <span class="signal-detail-label">Entry:</span>
                        <span class="signal-detail-value">${trade.entry_price ? parseFloat(trade.entry_price).toFixed(2) : 'N/A'}</span>
                    </div>
                    <div class="signal-detail">
                        <span class="signal-detail-label">SL:</span>
                        <span class="signal-detail-value">${trade.stop_loss ? parseFloat(trade.stop_loss).toFixed(2) : 'N/A'}</span>
                    </div>
                    <div class="signal-detail">
                        <span class="signal-detail-label">MFE:</span>
                        <span class="signal-detail-value">${trade.no_be_mfe ? parseFloat(trade.no_be_mfe).toFixed(2) + 'R' : 'N/A'}</span>
                    </div>
                    <div class="signal-detail">
                        <span class="signal-detail-label">BE:</span>
                        <span class="signal-detail-value">${trade.be_mfe && trade.be_mfe > 0 ? 'Yes' : 'No'}</span>
                    </div>
                </div>
            </div>
        `;
            ...
            html += `
                <div class="session-row">
                    ...
                    <div class="session-metrics">
                        <span class="session-metric ${avgR > 0 ? 'positive' : avgR < 0 ? 'negative' : ''}">${avgR.toFixed(2)}R</span>
                        <span class="session-metric">${(winRate * 100).toFixed(0)}%</span>
                        <span class="session-metric">${trades} trades</span>
                    </div>
                </div>
            `;
        }
    ...
    renderHotHours(sessionStats) {
        ...
        topSessions.forEach(([session, data]) => {
            ...
            html += `
                <div class="hot-hour-bar">
                    <span class="hot-hour-label">${session}</span>
                    <div class="hot-hour-fill" style="width: ${width}%"></div>
                    <span class="hot-hour-label">${avgR.toFixed(2)}R</span>
                </div>
            `;
        });
    ...
    renderSignalQuality() {
        ...
        if (confirmEl) {
            confirmEl.textContent = confirmTime.toFixed(0) + 'm';
        }
    ...
    /**
     * Render risk warnings based on H1 risk logic
     * Uses real data from this.data.risk and this.data.stats
     */
    renderRiskWarnings() {
        ...
        container.innerHTML = warnings.map(w => 
            `<div class="risk-warning ${w.type}">${w.message}</div>`
        ).join('');
    }
